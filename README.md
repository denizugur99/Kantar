...
**IMPORTANT:** This project uses environment variables for sensitive credentials. See [SECURITY.md](SECURITY.md) for detailed setup instructions.
...
For local development without Docker, you can configure `appsettings.Development.json` in the `Kantarv2` project directory.
...
**ÖNEMLİ:** Bu proje hassas kimlik bilgileri için ortam değişkenlerini kullanır. Detaylı kurulum talimatları için [SECURITY.md](SECURITY.md) dosyasına bakın.

Örnekten bir `.env` dosyası oluşturun:

```bash
cp .env.example .env
```

`.env` dosyasını düzenleyin ve gerçek kimlik bilgilerinizi doldurun:
- PostgreSQL şifresi
- RabbitMQ şifresi
...
**`.env` dosyasını asla git'e commit ETMEYİN!**

### 3. Altyapı Servislerini Başlatın
...
### 4. Uygulama Ayarlarını Yapılandırın (Opsiyonel)

Docker olmadan yerel geliştirme için, `Kantarv2` proje dizininde `appsettings.Development.json` dosyasını yapılandırabilirsiniz.
